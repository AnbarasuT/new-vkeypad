...
window.vKeyPad = function(TextBoxSelector, ParentContainerId, Options){
    ...
    this.onFocusTxtBoxBg = this.Options.onFocusTxtBoxBg ? this.Options.onFocusTxtBoxBg : "#ccc";
    ...
    this.divideSymbol = this.Options.divideSymbol ? this.Options.divideSymbol : "\u00F7";
...
vKeyPad.prototype = {
	...
	this.KeyPadHolder = $("<div class=\"keypadContainer\" style=\"display:inline-block; left:790px; top:210px;\"></div>");
	var vKey = this;
	...
	if (customKeys) {
		...
		this.addKeysTo(keys, this.eachManifest("hyphen",["_","&ndash;"], ["_",this.minusSymbol], false, 1, 1));
	    ...
	    this.addKeysTo(keys, this.eachManifest("2", "2", "2", false, 1, 1));
	    this.addKeysTo(keys, this.eachManifest("3", "3", "3", false, 1, 1));
	    this.addKeysTo(keys, this.eachManifest("4", "4", "4", false, 2, 1));
	    this.addKeysTo(keys, this.eachManifest("5", "5", "5", false, 2, 1));
	    this.addKeysTo(keys, this.eachManifest("6", "6", "6", false, 2, 1));
	    this.addKeysTo(keys, this.eachManifest("7", "7", "7", false, 3, 1));
	    this.addKeysTo(keys, this.eachManifest("8", "8", "8", false, 3, 1));
	    this.addKeysTo(keys, this.eachManifest("9", "9", "9", false, 3, 1));
	    this.addKeysTo(keys, this.eachManifest("0", "0", "0", false, 4, 1));
	    this.addKeysTo(keys, this.eachManifest("fullstop", ".", ".", false, 4, 1));
	    //this.addKeysTo(keys, this.eachManifest("equals","=", "=", false, 3, 1));
	   // this.addKeysTo(keys, this.eachManifest("italic1","<i>n</i>", "<i>n</i>", false, 4, 1));
	    //this.addKeysTo(keys, this.eachManifest("plus","+", "+", false, 4, 1));
	    //this.addKeysTo(keys, this.eachManifest("hyphen","&ndash;", this.minusSymbol, false, 4, 1));
	    //this.addKeysTo(keys, this.eachManifest("multi","&times;", this.multiSymbol, false, 4, 1));
	    //this.addKeysTo(keys, this.eachManifest("divide","&divide;", this.divideSymbol, false, 5, 1));
	    //this.addKeysTo(keys, this.eachManifest("sup","<i>x<sup>y</sup></i>", "sup", true, 5, 1));
	    //this.addKeysTo(keys, this.eachManifest("sub","<i>x<sub>y</sub></i>", "sub", true, 5, 1));
	}
	...
	return {"headerKeys":headerKeys, "contentKeys":keys, "footerKeys":footerKeys};
    },
	...
	if (key.hasClass("vkBtnKey") && key.attr('id') != "key_backspace") {
	...
	if (vK.CurrentFocus && keyDetails) {
		...
		if (keyText == "enter"){
			...
			if(lastChar == ";" && this.isTaggableBox){
			    var lastEntity = existingText.substr(existingText.lastIndexOf("&"));
			    if (lastEntity.length > 3 && lastEntity.indexOf(" ") < 0 && lastEntity.indexOf(";") == lastEntity.length-1)
				existingText = existingText.substr(0, existingText.length-lastEntity.length);
			    else existingText = existingText.substr(0, existingText.length-1);
			}
			else if(lastChar == ">" && this.isTaggableBox) {
			    var lastTag = existingText.substr(existingText.lastIndexOf("<"));
			    var tagName = lastTag.replace(/[\<|\>|/]/g, "");
			    var tagsReg = new RegExp("\\<"+tagName+".*?\\>.*?\\</"+tagName+"\\>","g");
			    var tagMatch =  existingText.match(tagsReg);
			    var breakMatch = lastTag.match(/\<br(\s)*(\/)*\>/g);
			    if (breakMatch)
				existingText = existingText.substr(0, existingText.length-lastTag.length);
			    else if (tagMatch){
				var wholeTag = tagMatch[tagMatch.length-1];
				var tagIndex = existingText.lastIndexOf(wholeTag);
				var remTag = false;
				if (this.deleteMethod == "byChar"){
				    var textIndex = existingText.lastIndexOf("<");
				    if (textIndex > tagIndex && textIndex > 0) {
					var charIndex = textIndex-1;
					if(existingText[charIndex] != ">"){
					    existingText = existingText.substr(0, textIndex-1)+existingText.substr(textIndex);
					    if (tagName == "sup") {
						this.isSup = true;
						this.isSub = false;
					    }
						...
						//this.isItalic = true;
						//this.isBold = false;
						existingText = existingText.substr(0, tagIndex);
					    }
					    else if (tagName == "b") {
						this.isBold = true;
						this.isItalic = false;
				...
				if (this.deleteMethod == "byTag" || remTag) {
				    disSupSub = true;
				    disItaBol = true;
				    existingText = existingText.substr(0, tagIndex);
				}
				...
				disSupSub = true;
				disItaBol = true;
				existingText = existingText.substr(0, existingText.length-1);
			    }
			}
			else {
			    disSupSub = true;
		...
		if (this.textLength(existingText) < vK.maximumLength || vK.maximumLength == 0) {
			...
			if (pass) {
				...
				$(vK.CurrentFocus).text(displayVal);
	...
	var textLen = textElem.text().length;
	textElem.remove();
	return textLen;
    },
    getText:function(){
    ...
    blur:function(){
	var vK = this.getSelf();
	if (this.textBoxOrgBg && vK.CurrentFocus) $(vK.CurrentFocus).css({"background": "#fffed2"});
	if (vK.CurrentFocus){
    ...
    onTextChanged:function(){reset_function()}
};
